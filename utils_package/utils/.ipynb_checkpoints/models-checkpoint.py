...
class ProtoNets(torch.nn.Module):
    ...
    def forward(self, episode):
        ...
        class_count = max([int(a[1:]) for a in support_labels]) + 1

        class_iterator = range(class_count)

        support_labels = support_labels + ['Q'] * len(query)
        datapoints = support + query

        datapoints = self.encoder(datapoints)

        class_vectors = []

        # Average each support set class
        for i in class_iterator:
            index = ['S' + str(i) == a for a in support_labels]
            index = [i for i, a in enumerate(index) if a]

            support_vectors = datapoints[index, ...]

            class_prototype = support_vectors.mean(0)
            class_vectors.append(class_prototype)
        ...
        return -euclidean_distance(query, class_vectors)
